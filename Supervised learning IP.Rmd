...
#advertising<- data.frame(advertising)
...
ad_model <- select(advertising, c('Daily_spent_site', 'Age', 'Area_income', 'Daily_internet_use', 'Male', 'Clicked_ad'))
...
rand <- sample(1:nrow(ad_model),0.9 * nrow(ad_model))
...
rand_norm <- as.data.frame(lapply(ad_model[,c(1:5)], norm))
 
# The training dataset extracted
ad_model_train <- rand_norm[rand,]
 
# The test dataset extracted
ad_model_test <- rand_norm[-rand,]

```

```{r}
summary(ad_model_test)
...
#train <- ad_model_train[rand,]
#test <- ad_model_test[-rand,]

train_target <- as.factor(advertising[rand,10])
test_target <- as.factor(advertising[-rand,10])
...
library(class)
pr <- knn(ad_model_train,ad_model_test,cl=train_target,k=20)
...
```
KNN accuracy score is 97%



#Challenging solution with SVM
```{r}
...
```{r}
# 
ad_train <- createDataPartition(y =ad_model$Clicked_ad, p= 0.7, list = FALSE)
training <- ad_model[ad_train,]
testing <- ad_model[-ad_train,]
...
ad_model_train <- data.frame(ad_model_train)
...
# 
training[["Clicked_ad"]] = factor(training[["Clicked_ad"]])

svm_Linear <- train(Clicked_ad ~., data = training, method = "svmLinear",
trControl=trctrl,
...
```{r}
confusionMatrix(table(test_pred, testing$Clicked_ad))
...
The accuracy score for KNN algorithm is 97% while that of SVM is 97.6%.
...
#Recommendations
The most suitable model to use will be SVM.
